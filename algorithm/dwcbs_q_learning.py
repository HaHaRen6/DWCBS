...
from algorithm.q_learning_agent import RLAgent
import pickle
from collections import deque, defaultdict
import copy

WRITE_WINDOW_SIZE = True
WRITE_NORM = True
DEBUG = False
time_step_count = 0
...
class DWCBSRLSolver(object):
    ...
    def __init__(self, my_map, starts, goals, train_flag=False, max_time=60):
        """
        ...
        self.action_space = [0, 1, -1, 2, -2, 3]
        self.rl_agent = RLAgent(self.action_space)

        # if not self.train_flag:
        try:
            self.rl_agent.load("q_table/q_table.pkl")
            self.old_q_table = copy.deepcopy(self.rl_agent.q_table)
        ...
        while self.open_list and timer.time() - self.start_time < self.max_time:
            ...
            if not p['collisions']:
                if self.all_agents_reached_goals(p['paths'], p['time_window'][1]):
                    ...
                    L1, L2 = RLAgent.compute_q_table_distance(self.rl_agent.q_table, self.old_q_table)
                    print("L1: " + str(L1) + "\tL2: " + str(L2))
                    if WRITE_NORM:
                        normfile = open("norm.csv", "a", buffering=1)
                        normfile.write(str(L1) + "," + str(L2) + "\n")
                        normfile.close()
                    self.rl_agent.save("q_table/q_table.pkl")
                    ...
                    new_window_size = self.init_window_size if action == 3 else max(current_window_size + action, 2)
                    ...
                    if self.train_flag:
                        ...
                        reward = (
                            ...
                            + new_window_size * self.rl_agent.a3
                        )
