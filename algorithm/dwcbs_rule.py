...
class DWCBSSolver(object):
    ...
    def __init__(self, my_map, starts, goals, dwindowmin, dwindowstep, max_time=30):
        """
    ...
    def push_node(self, node):
        heapq.heappush(self.open_list, (node['cost'], len(node['collisions']), node['time_window'][0]-node['time_window'][1],-node['time_window'][1], self.num_of_generated, node))
        if DEBUG:
    ...
    def find_solution(self):
        ...
        # windowsizefile = open("windowsize.csv", "w", buffering=1)
        while self.open_list and timer.time() - self.start_time < self.max_time:
            ...
            if not p['collisions']:
                if self.all_agents_reached_goals(p['paths'], p['time_window'][1]):
                    ...
                    # beg = p['time_window']
                    window_size = p['time_window'][1] - p['time_window'][0] + 1 + self.dwindowstep
                    # windowsizefile.write(str(window_size)+"\n")
                    # print(window_size)
                    # if(window_size >=5):
                    #     print(window_size)
